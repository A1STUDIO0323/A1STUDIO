...
function KakaoPaySuccessContent() {
  ...
  useEffect(() => {
    const run = async () => {
      if (!pgToken || !orderId || !partnerUserId) {
      ...
      try {
        const res = await fetch("/api/payments/kakao/approve", {
          ...
          body: JSON.stringify({ pgToken, orderId, partnerUserId }),
        });
        const data = (await res.json()) as ApproveResult & { error?: string };
        if (!res.ok) {
      ...
      <div className="mx-auto w-full max-w-md rounded-2xl border border-[#D8CCBC] bg-[#EFE7DA] p-8 text-center">
        {loading ? (
          ...
          <div>
            ...
            <h1 className="mt-3 text-xl font-bold text-[#3B342F]">카카오페이 결제 완료</h1>
