...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user?.id) {
      return NextResponse.json({ success: false, error: "Î°úÍ∑∏???∏ÏÖò???ÜÏäµ?àÎã§." }, { status: 401 });
    }
    ...
    if (isDbConnectionError(error)) {
      // Î°úÍ∑∏???ôÍ∏∞?îÎäî Î∂ÄÍ∞Ä Í∏∞Îä•?¥Î?Î°?DB ?•Ïï† ???îÏ≤≠???±Í≥µ?ºÎ°ú Ï≤òÎ¶¨???¨Ïö©???êÎ¶Ñ??ÎßâÏ? ?äÏäµ?àÎã§.
      return NextResponse.json({ success: true, skipped: true, reason: "DB_UNAVAILABLE" });
    }
    console.error("[POST /api/members/sync]", error);
    return NextResponse.json({ success: false, error: "?åÏõê ?ôÍ∏∞???§Ìå®" }, { status: 500 });
  }
}
