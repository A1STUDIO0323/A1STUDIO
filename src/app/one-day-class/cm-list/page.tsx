...
// 원데이클래스 CM 목록 (공개 페이지)
// - cm_applications.status='APPROVED' + can_oneday=true 인 CM
// - cm_profiles.is_public=true + is_active=true
// - Phase 2에서 show_in_list 컬럼이 추가되면 추가 필터됨
export default function OneDayClassCmListPage() {
  return (
      ...
      <div className="mx-auto w-full max-w-5xl">
        <div className="mb-8 flex items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-extrabold text-[#3B342F]">원데이클래스 CM 목록</h1>
            <p className="mt-2 max-w-2xl text-sm text-[#6f655d]">
              원데이클래스를 진행할 수 있는 승인된 클래스마스터를 소개합니다.
            </p>
          </div>
          <Link
            href="/one-day-class"
        ...
        <CmCardSection type="oneday" variant="list" />
      </div>
    </div>
  );
}
