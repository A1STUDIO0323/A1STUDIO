...
### 10-12. 상세/설명 페이지는 썸네일 갤러리형 2단 레이아웃 기본

비품·상품 등 개별 항목의 상세 페이지/설명 페이지를 새로 만들 때는 **썸네일 갤러리형 2단 레이아웃**을 기본으로 사용:

- **왼쪽**: 큰 대표 사진 + 하단 썸네일 갤러리 (썸네일 클릭 시 대표 사진 전환, 이미지 없으면 아이콘 플레이스홀더 폴백)
- **오른쪽 상단**: 제품명(+모델명·수량·상태·요금 등 메타 정보)
- **오른쪽 하단**: 사용방법/상세 설명

참조 구현: `src/components/equipment/EquipmentDetailView.tsx` (`/equipment/[id]`). 목록 페이지에서는 컴팩트 카드 + "사용방법" 버튼으로 상세 페이지로 이동.
