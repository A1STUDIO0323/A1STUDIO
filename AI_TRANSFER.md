...
### 10-13. 🔒【절대지침】확장 대비 설계 원칙 (일 두 번 하지 않기)

**적용 대상: A1STUDIO 및 이후 모든 개발 프로젝트.** 로드맵상 "나중에 추가"가 확정된 기능은 MVP/초기 설계 때부터 같은 시스템 위에 올라가도록 설계하고, 추가 시점에는 UI/설정만 여는 것을 원칙으로 한다.

1. **종류가 늘어날 수 있는 개념은 하드코딩 금지, 데이터로 관리** — 카테고리·요금제·인증/배지 종류·문답 항목 등 "종류가 추가될 수 있는 것"은 테이블/설정 행으로 관리. 확장 = 행 추가, 코드 수정 없음
2. **확정 로드맵의 미래 필드는 첫 스키마에 선반영** — 공개 범위·등급 구분 등은 첫날부터 컬럼으로 넣고 기본값 고정으로 시작. 나중에 넣으면 대규모 마이그레이션이 되는 지점을 우선 선반영
3. **기능 접근 제어는 요금제/등급 테이블 + 기능 플래그 구조로 통일** — 단일 요금제여도 티어 추가가 행 추가가 되게
4. **공통 흐름(검수·알림·만료 등)은 종류별로 복제하지 말 것** — 공통 로직 하나가 데이터 기반으로 처리하게 설계

### 10-14. 🔒【절대지침】변경 전파 스윕

결정·정책·명칭이 변경되거나 폐기되면, 한 곳만 고치지 말고 **문서·코드 전체를 grep으로 스윕해 모든 참조를 일괄 정리**할 것.

- 문서의 경우 확정 목록·미결정 목록·아이디어 목록 등 파생 언급까지 모두 확인
- **배경**: 폐기된 항목이 다른 섹션 목록에 남아 사용자가 직접 발견한 사례 재발 방지 (2026-07-04)
